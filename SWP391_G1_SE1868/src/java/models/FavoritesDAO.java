 ...
 * @author Giang123
 */
public class FavoritesDAO extends GiangDBcontext{
    private static final Logger LOGGER = Logger.getLogger(ProductDAO.class.getName());
    
    // Thêm phương thức tìm kiếm vào class FavoritesDAO
public List<Favorite> searchFavorites(int customerId, String searchQuery, int page, int pageSize) {
    List<Favorite> searchResults = new ArrayList<>();
    ...
    String sql = """
                 ...
                 WHERE 
                     f.CustomerID = ? 
                     AND (LOWER(p.Name) LIKE ? OR LOWER(s.Name) LIKE ?)
                 LIMIT ? OFFSET ?""";
    ...
    try (Connection conn = getConnection(); 
        ...
        try (ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                ...
                // Format price using FormatUtils
                favorite.setFormattedPrice(FormatUtils.formatCurrency(favorite.getPrice()));
                searchResults.add(favorite);
            }
        }
    } catch (SQLException e) {
        LOGGER.log(Level.SEVERE, "Lỗi khi tìm kiếm sản phẩm yêu thích", e);
    }
    return searchResults;
}

public int getTotalSearchResults(int customerId, String searchQuery) {
    String sql = """
                 SELECT COUNT(*) 
                 FROM Products p 
    ...
    try (Connection conn = getConnection();
        ...
        LOGGER.log(Level.SEVERE, "Lỗi khi đếm kết quả tìm kiếm", e);
    }
    return 0;
}
